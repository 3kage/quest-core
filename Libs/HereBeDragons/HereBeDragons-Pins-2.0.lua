...
WorldMapFrame.pinPools["HereBeDragonsPinsTemplateQC"] = worldmapPinsPool

-- provider base API
function worldmapProvider:RemoveAllData()
    self:GetMap():RemoveAllPinsByTemplate("HereBeDragonsPinsTemplateQC")
end

function worldmapProvider:RemovePinByIcon(icon)
    for pin in self:GetMap():EnumeratePinsByTemplate("HereBeDragonsPinsTemplateQC") do
...
function worldmapProvider:HandlePin(icon, data)
    ...
    if x and y then
        self:GetMap():AcquirePin("HereBeDragonsPinsTemplateQC", icon, x, y, data.frameLevelType)
    end
end
