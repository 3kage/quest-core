...
data.basenodes.FloorCrossings = --[[LIBROVER]] {
	...
	-- MIDNIGHT
		["Naigtal"] = {
			"Naigtal/0 28.10,50.66 <radius:7> -x- Vilaldoun/0 70.03,22.76 <radius:7>"..
				"{title_atob:Enter the crypt} {title_btoa:Leave the crypt}",
		},

		["Isle of Quel Danas M"] = {
			"Isle of Quel Danas M/0 64.01,29.12 <radius:7> -x- The Lycaneum/0 64.22,87.72 <radius:10>"..
				"{title_atob:Enter the building} {title_btoa:Leave the building}",
		},

		["Isle of Quel Danas (Intro)"] = {
			"Isle of Quel Danas (Intro)/0 48.49,38.37 <radius:30> -x- Isle of Quel Danas (Intro)/1 51.17,50.07 <radius:30>"..
				"{title:Travel within the intro zone}",
			"Isle of Quel Danas (Intro)/0 48.49,38.37 <radius:30> -x- Isle of Quel Danas (Intro)/2 50.00,50.00 <radius:30>",
			"Isle of Quel Danas (Intro)/1 51.17,50.07 <radius:30> -x- Isle of Quel Danas (Intro)/2 50.00,50.00 <radius:30>",
		},

		["Void Acropolis"] = {
			"Void Acropolis/1 86.36,75.06 <radius:10> -x- Void Acropolis/0 75.15,71.39 <radius:10>"..
				"{title_atob:Run down the stairs} {title_btoa:Run up the stairs}",
			--"Void Acropolis/0 20.75,31.60 <radius:10> -x- Void Acropolis/1 15.55,22.25 <radius:10>"..
				--"{title_atob:Run up the stairs} {title_btoa:Run down the stairs}",
		},

		["Forgotten Depths"] = {
			"Forgotten Depths/0 27.23,56.12 <radius:10> -x- Forgotten Depths/1 32.43,37.45 <radius:10>"..
				"{title_atob:Run up the path} {title_btoa:Run down the path}",
		},

		["Voidstorm"] = {--Lair of Predaxas Voidburrow
			"Voidstorm/0 48.14,78.63 <radius:7> -x- Lair of Predaxas/1 60.00,17.51 <radius:10>"..
				"{title_atob:Follow the Path Down and Enter\nthe Cave } {title_btoa:Follow the Path Up\nand Leave the Cave}",		-- Lair of Predaxas Cave
			"Voidstorm/0 39.06,53.36 <radius:7> -x- Voidburrow/0 42.39,23.20 <radius:10>"..
				"{title_atob:Follow the Path Down and Enter\nthe Cave } {title_btoa:Follow the Path Up\nand Leave the Cave}",		-- Voidlands Abundance Cave
		},

		["Voidburrow"] = {--Voidburrow
			"Voidburrow/0 29.55,18.35 <radius:10> -x- Voidburrow/1 30.87,29.57 <radius:15>",		-- Voidburrow Cave upstairs/downstairs
			"Voidburrow/1 54.54,21.62 <radius:10> -x- Voidburrow/0 42.98,16.41 <radius:15>",		-- Voidburrow Cave upstairs/downstairs
		},

		["Lair of Predaxas"] = {--Voidburrow
			"Lair of Predaxas/1 68.68,67.48 <radius:10> -x- Lair of Predaxas/0 73.53,43.29 <radius:10>",		-- Lair of Predaxas Cave upstairs/downstairs
		},

		["Harandar"] = {
			"Harandar/0 66.17,61.69 <radius:15> -x- Floaret Grotto/0 31.91,85.85 <radius:15>"..
				"{title_atob:Enter the Cave After\nTalking to Dundun} {title_btoa:Leave the Cave}",		-- Floaret Grotto
		},

		["Floaret Grotto"] = {
			"Floaret Grotto/0 43.60,66.14 <radius:8> -x- Floaret Grotto/1 37.73,57.44 <radius:8>",		-- Floaret Grotto upstairs/downstairs
		},

		["Zul Aman M"] = {
			"Zul Aman M/0 39.74,23.18 <radius:30> -x- Wit'Kalar Crypt/0 73.70,78.49 <radius:15>"..
				"{title_atob:Enter the Crypt} {title_btoa:Leave the Crypt}",		-- Wit'Kalar Crypt
			"Zul Aman M/0 22.02,63.42 <radius:30> -x- Revantusk Sedge/0 83.89,39.15 <radius:15>"..
				"{title_atob:Enter the Building} {title_btoa:Leave the Building}",		-- Revantusk Sedge
			"Zul Aman M/0 31.60,26.11 <radius:15> -x- Loaknit Den/0 74.50,68.45 <radius:15>"..
				"{title_atob:Enter the Cave After\nTalking to Dundun} {title_btoa:Leave the Cave}",		-- Loaknit Den
		},

		["Eversong Woods M"] = {
			"Eversong Woods M/0 56.76,65.79 <radius:10> -x- Wartha'nan Crypts/0 88.60,34.75 <radius:10>"..
				"{title_atob:Enter the Crypt After\nTalking to Dundun} {title_btoa:Leave the Crypt}",		-- Watha'nan Crypts entrance
			"Wartha'nan Crypts/0 43.98,84.98 <radius:10> -to- Eversong Woods M/0 54.76,68.05 <radius:10>"..
				"{title:Leave the Crypt}}",		-- Watha'nan Crypts entrance (exit only, causes issues two-way)
		},
		...
		["Dalaran L"] = {
			-- Underbelly Entrance for Warlock Order Hall Portal
				"Dalaran L/10 34.33,45.60 <radius:10> -x- Dalaran L/11 19.18,57.14 <radius:10>"..
					...
					"{title_btoa:Run up the stairs and leave the Underbelly} {cond:QC.IsLegionOn()}",
		...
		["Val'sharah"] = {					
			-- The Dreamgrove Ground Entrance (Druid Only)
				"Val'sharah/0 45.48,34.51 <radius:20> -x- The Dreamgrove/0 44.82,32.76 <radius:20>"..
					...
					"{cond:select(2,UnitClass('player'))=='DRUID' and QC.IsLegionOn()}",
		},
		...
		["Highmountain"] = {
			-- Trueshot Lodge Ground Entrance (Hunter Only)
				"Highmountain/0 36.13,44.71 <radius:20> -x- Trueshot Lodge/0 42.72,9.98 <radius:20>"..
					...
					"{cond:select(2,UnitClass('player'))=='HUNTER' and QC.IsLegionOn()}",
		},
